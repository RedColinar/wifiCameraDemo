...
public class BitmapUtils {
    ...
    public static Bitmap getImageByPath(String imagePath, int width, int height) {
        Log.d(TAG, "Start getImageByPath imagePath=" + imagePath);
        if (imagePath == null) {
            return null;
        }
        Bitmap bitmap = null;
        ...
        Log.d(TAG, "End getImageByPath bitMap=" + bitmap);
//        AppLog.d( TAG, "End getImageByPath bitMap.getByteCount()=" + bitmap.getByteCount() );
    ...
    public static int calculateInSampleSize(BitmapFactory.Options options, int reqWidth, int reqHeight) {
        ...
        Log.d(TAG, "height * width /(inSampleSize * inSampleSize) *4 =" + height * width / (inSampleSize * inSampleSize) * 4);
        while (height * width / (inSampleSize * inSampleSize) * 4 > LIMITED_IMGAE_SIZE) {//out of memory.
            inSampleSize *= 2;
        }
        Log.d(TAG, "calculateInSampleSize return inSampleSize=" + inSampleSize);
        return inSampleSize;
    }
    ...
    public static Bitmap getVideoThumbnail(String videoPath, int width, int height) {
        Log.i(TAG, "start getVideoThumbnail videoPath=" + videoPath);
        if (videoPath == null) {
            return null;
        }
        Bitmap bitmap = null;
        ...
        Log.i(TAG, "End getVideoThumbnail bitmap=" + bitmap);
        return bitmap;
    }
    ...
    public static Bitmap zoomBitmap(Bitmap bitmap, float width, float heigth) {
        ...
        try {
            ...
            Log.e(TAG, "zoomBitmap OutOfMemoryError");
            tempBitmap.recycle();
        }
        bitmap.recycle();
        System.gc();
    ...
    public static Bitmap decodeByteArrayForRGB565(byte[] data, int reqWidth, int reqHeight) {
        Log.d(TAG, "start decodeByteArrayForRGB565");
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
...
//        options.inSampleSize = 2;
        ...
        Log.d(TAG, "end decodeByteArrayForRGB565 bitmap=" + bitmap);
        return bitmap;
    }

    public static Bitmap decodeByteArray(byte[] data, int reqWidth, int reqHeight) {
        Log.d(TAG, "start decodeByteArray");
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        BitmapFactory.decodeByteArray(data, 0, data.length, options);
        int sampleSize = calculateInSampleSize(options, reqWidth, reqHeight);
...
//        return bitmap;
    ...
    public static byte[] bitmapToByteArray(Bitmap bitmap) {
        ...
        Log.d(TAG, "bitmapToByteArray bitmap size=" + bitmap.getByteCount());
        ...
        if (result != null) {
            Log.d(TAG, "bitmapToByteArray result size=" + result.length);
        }
